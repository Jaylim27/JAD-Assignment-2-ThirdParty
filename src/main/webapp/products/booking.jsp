...
<style>
/* (STYLE mostly same as yours; only layout adjustments are commented below) */
:root {
  --bg: #f6f8fb;
  --card: #ffffff;
  --text: #1f2937;
  ...
  <div class="container">
    ...
    <%
      ...
      if (productId == null || productId.trim().isEmpty()) {
        Object productIdObj2 = request.getAttribute("productId");
        productId = (productIdObj2 != null) ? String.valueOf(productIdObj2) : "";
      }
    %>
    ...
    <div class="crumbRow">
      <div class="crumbLeft">
        <div class="crumb">
          ...
          <a href="<%=request.getContextPath()%>/GetProductList">Services</a> /
          <a href="<%=request.getContextPath()%>/GetProductList?productId=<%=productId%>">Details</a> /
          Booking
        </div>
      </div>

      <a class="btn ghost"
         href="<%=request.getContextPath()%>/GetProductList?productId=<%=productId%>">← Back to details</a>
    </div>
    ...
    <%
      ...
      /* (kept) your attribute fallback; not required now but harmless */
    ...
    <%
      ...
      if (err != null) {
        ...
        if ("NotFound".equals(err)) msg = "Service not found.";
        if ("InvalidId".equals(err)) msg = "Invalid Service ID.";
        if ("ApiError".equals(err)) msg = "Unable to load service details from API.";
    %>
      <div class="msg error"><%=msg%></div>
      <a class="btn primary" href="<%=request.getContextPath()%>/GetProductList">Return to services</a>
    <%
    ...
    <div class="bookingGrid">
      <div class="card">
        <div class="heroTop">
          ...
          <div class="idTag">Service ID: <%=product.getProductId()%></div>
        </div>

        <div class="cardBody">
          <div class="titleRow">
            <h1>Booking: <%=product.getName()%></h1>
          </div>
